...
class Pkcs11 {
    public:
        ...
        CK_SESSION_HANDLE m_ulObjectHandle;
        int m_ulSlotId;
        ...
        void startSession(unsigned long ulSlot, bool fRW=true);
        void logout(); 
        bool needsLogin(bool fAsSecurityOfficer);
        void login(const unsigned char* pcPin, unsigned long ulPinlen, bool fAsSecurityOfficer);
        void setPin(const unsigned char* pcOldPin, unsigned long ulOldPinLen, const unsigned char* pcPin, unsigned long ulPinLen);
        void initPin(const unsigned char* pcPin, unsigned long ulPinLen);
        QStringList tokenInfo(CK_SLOT_ID ulSlotId) const;
        QStringList tokenInfo() const;
        bool protectedAuthenticationPath(CK_SLOT_ID ulSlotId);
        bool protectedAuthenticationPath();
        void loadAttribute(Pkcs11Attribute& attribute, CK_OBJECT_HANDLE ulObjectHandle) const;
        void storeAttribute(Pkcs11Attribute& attribute, CK_OBJECT_HANDLE ulObjectHandle);
        CK_OBJECT_HANDLE createObject(const Pkcs11Attlist& attrs);
        QList<CK_OBJECT_HANDLE> objectList(const Pkcs11Attlist& atts);

        static bool loadLibrary(const QString& strFilePath, bool fSilent);
        //static bool loaded() {return _loaded;};
        static bool loaded() {return !!m_pLoadedModuleHandle;};
        static bool closeLibrary(const QString& strFilePath, bool fSilent);
        static QList<unsigned long> slotList();
        static void pk11error(const QString& strfunc, int iReturnValue);
        static unsigned long slotsAvailable();
        static QList<CK_MECHANISM_TYPE> mechanismList(unsigned long ulSlot);
